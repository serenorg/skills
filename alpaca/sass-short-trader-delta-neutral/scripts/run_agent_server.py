...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path != "/health":
            ...
            return
        json_response(self, 200, {"status": "ok", "service": "sass-short-trader-delta-neutral"})
    ...
    def log_message(self, fmt: str, *args: object) -> None:
        print(f"[sass-short-trader-delta-neutral] {self.address_string()} - {fmt % args}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run sass-short-trader-delta-neutral webhook server")
...
def main() -> None:
    ...
    if not args.webhook_secret:
        raise SystemExit("SASS_SHORT_TRADER_DELTA_NEUTRAL_WEBHOOK_SECRET is required (--webhook-secret or env)")
